...
function PropertiesRemove({ open, onClose, openSnackBar, propertyKey: key, onSuccess }) {
  const onRemove = () => {
    api
      .removePropertiesConfiguration({ key })
      .then(() => {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth={false}>
      <DialogTitle>{`Remove Configuration Property "${key}"?`}</DialogTitle>
      <DialogActions>
