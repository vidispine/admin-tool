...
export default function ScheduledRequestDetail({ scheduledRequestType, openSnackBar }) {
  const { id: requestId, request = {}, response = {} } = scheduledRequestType;
  const { contentType } = response;
  const [responseBody, setResponseBody] = useState();
  const onGetScheduledRequestResponse = () => {
    if (!response.hasBody) return;
    api
      .getScheduledRequestResponse({ requestId })
  ...
  useEffect(onGetScheduledRequestResponse, [openSnackBar, requestId, response.hasBody]);
  return (
    <>
      <TextGrid title="URI" value={request.uri} hover />
      <TextGrid title="Method" value={request.method} hover />
      <TextGrid title="Request Body" value={request.body} variant="code" hover hideNoValue />
      {response.statusCode && (
        <>
          ...
          {responseBody && (
            <TextGrid
              title="Response Body"
              value={
                contentType === 'application/json' ? JSON.stringify(responseBody) : responseBody
              }
              variant={contentType}
              hover
              hideNoValue
