...
function TaskDefinitionRemove({ open, onClose, openSnackBar, taskDefinitionDocument, onSuccess }) {
  const { step, jobType } = taskDefinitionDocument;
  const onRemove = () => {
    api
      .removeTaskDefinitionStep({ taskDefinitionType: jobType, taskDefinitionStep: step })
      .then(() => {
