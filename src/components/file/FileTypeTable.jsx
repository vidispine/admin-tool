...
function FileTypeRow({ fileType }) {
  return (
    <TableRow key={fileType.id} hover>
      <TableCell>
        <UnstyledLink to={`/file/${fileType.id}/`}>{fileType.id}</UnstyledLink>
      </TableCell>
      ...
      <TableCell>
        <UnstyledLink to={`/storage/${fileType.storage}/`}>{fileType.storage}</UnstyledLink>
      </TableCell>
    </TableRow>
  );
}

export default function FileTypeTable({ value, hideNoValue, title = 'Files' }) {
