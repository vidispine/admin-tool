...
function ItemSequenceRemove({
  ...
  const onRemove = async () => {
    try {
      ...
      const messageContent = `Sequence ${format} Removed`;
      if (openSnackBar) openSnackBar({ messageContent });
      if (onClose) onClose();
      if (onSuccess) onSuccess();
    } catch (error) {
      const messageContent = 'Error Removing Sequence';
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth={false}>
      <DialogTitle>{`Remove Sequence "${format}"?`}</DialogTitle>
      <DialogActions>
...
export default withUI(ItemSequenceRemove);
